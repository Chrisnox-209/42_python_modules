...
class DataStream(ABC):
    def __init__(self, stream_id: str) -> None:
        ...
        self.type_sensor: str = "type_sensor"
...
class SensorStream(DataStream):
    def __init__(self, stream_id: str) -> None:
        ...
        self.type_sensor = "Environmental Data"

    def process_batch(self, data_batch: List[Any]) -> str:
        self.items_processed += len(data_batch)
        temp_str: Any = data_batch[0].split(":")[1]
        temp = float(temp_str)
...
class TransactionStream(DataStream):
    def __init__(self, stream_id: str) -> None:
        super().__init__(stream_id)
        self.type_sensor = "Financial Data"

    def process_batch(self, data_batch: List[Any]) -> str:
        self.items_processed += len(data_batch)
        list_transction: list = []
        for data in data_batch:
            if data.split(":")[1].isdigit():
                list_transction.append(int(data.split(":")[1]))
        reult: Any = ((list_transction[0] - list_transction[1]) +
                      list_transction[2])
        return (f"Stream ID: {self.stream_id}, Type: {self.type_sensor}\n"
                f"Processing sensor batch: [{', '.join(data_batch)}]\n"
                f"ransaction analysis: {len(data_batch)} operations, "
                f"net flow: +{reult} units")


class EventStream(DataStream):
    def __init__(self, stream_id: str) -> None:
        super().__init__(stream_id)
        self.type_sensor = "System Events"
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        ...
        return (f"Stream ID: {self.stream_id}, Type: {self.type_sensor}\n"
                ...
                f"{sum(1 for data in data_batch if data == 'error')} "
                "error detected")
    ...
    def process_all(self, batches_dict: Dict[str, List[Any]]) -> None:
        ...
        for stream in self.streams:
            if stream.stream_id in batches_dict:
                try:
                    data_batch: List[Any] = batches_dict[stream.stream_id]
                    stream.process_batch(data_batch)
                    stats: Dict[str, str | int | float] = stream.get_stats()
                    if "Environmental" in str(stats["type"]):
                        ...
                        word = "events"
                        prefix = "Event"
                    print(f"- {prefix} data: {len(data_batch)} {word} "
                          "processed")
...
if __name__ == "__main__":
    ...
    my_sensor_00 = SensorStream("SENSOR_001")
    data_sensor_00: List[str] = [
        "temp:22.5",
        "humidity:65",
        "pressure:1013"
        ]
    ...
    my_sensor_01 = TransactionStream("TRANS_001")
    data_sensor_01: List[str] = [
        "buy:100",
        "sell:150",
        "buy:75"
        ]
    ...
    my_sensor_02 = EventStream("EVENT_001")
    data_sensor_02: List[str] = [
        "login",
        "error",
        "logout"
        ]
    ...
    raw_sensor_data: List[str] = [
        "temp:22",
        "alert:critical_temp",
        "humidity:50",
        "alert:critical_pressure"
        ]
    raw_transaction_data: List[str] = [
        "buy:10",
        "sell:50",
        "buy:10000",
        "buy:20"
        ]
    critical_sensors: List[Any] = my_sensor_00.filter_data(
        raw_sensor_data,
        criteria="critical"
    )
    large_transactions: List[Any] = my_sensor_01.filter_data(
